...
実証用の最小推奨VLMは Ollama の `gemma3:4b` です。理由は、ローカル実行でき、Text+Image入力に対応し、約3.3GBの小さい構成で、多言語プロンプトの反復比較に使いやすいためです。

より強いローカル比較が必要な場合は `gemma3:12b` を使います。`mock` は分類精度の検証ではなく、パイプライン接続確認専用です。
...
選定根拠は `docs/model_selection.md`、実験手順は `docs/research_protocol.md` を参照してください。
...
- `ollama`: 実証用の主backendです。`OLLAMA_ENABLED=true`, `OLLAMA_HOST`, `OLLAMA_MODEL=gemma3:4b` を設定します。
...
サンプリング方式とマスク方式の比較実験をまとめて実行できます。
